module.exports = async function handler(req, res) {
  ...
  const openaiModel = (body && body.openaiModel) ? body.openaiModel : 'gpt-4o';
  ...
  // ══════════════════════════════════════════
  async function runClaude(sharedSearch) {
    ...
    const d = await r.json();
    return { ok:true, analysis:d.content[0].text, provider:'claude-sonnet-4-5', webSearch:!!sharedSearch, searchSource:'Tavily' };
  }
  ...
  async function runGPT(sharedSearch, model='gpt-4o') {
    ...
    const d = await r.json();
    return { ok:true, analysis:d.choices[0].message.content, provider:'openai-'+model, webSearch:!!sharedSearch, searchSource:'Tavily' };
  }
  ...
  try {
    ...
    if (forceClaude) {
      const r = await runClaude(sharedSearch);
    ...
    if (claudeKey) {
      try { const r=await runClaude(sharedSearch); res.status(200).json(r); return; }
