module.exports = async function handler(req, res) {
  ...
  const openaiModel = (body && body.openaiModel) ? String(body.openaiModel).trim() : 'gpt-4o';
  ...
  const ALLOWED_GPT_MODELS = ['gpt-4o','gpt-4o-mini','gpt-4.1','gpt-4.1-mini','gpt-4.5','gpt-4.5-preview'];
  const validatedOpenAIModel = ALLOWED_GPT_MODELS.includes(openaiModel) ? openaiModel : 'gpt-4o';
  ...
  async function runGPT(sharedSearchData, model = 'gpt-4o') {
    ...
    return {
      ...
      model: model,
      model_validated: validatedOpenAIModel,
      webSearch: !!sharedSearchData,
      searchSource: 'Tavily'
    };
  }
  ...
  try {
    ...
    if (forceClaude && forceGPT) {
      ...
      const [rC, rG] = await Promise.allSettled([
        ...
        runGPT(sharedSearch, validatedOpenAIModel)
      ]);
    ...
    if (forceGPT) {
      const queries = buildQueries(basePrompt, matchData);
      const search = await tavilySearch(queries);
      const r = await runGPT(search, validatedOpenAIModel);
    ...
    if (openaiKey) {
      try { const r = await runGPT(search, validatedOpenAIModel); if(r){res.status(200).json(r);return;} } catch(e) {}
